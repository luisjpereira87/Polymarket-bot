...
import { ClobApiKeyCreds, ConnectionStatus, Message, RealTimeDataClientConfig, RealTimeDataClientInterface, SubscriptionMessage, WS_ENDPOINTS } from './types.js';
...
export class RealTimeDataClient implements RealTimeDataClientInterface {
    ...
    private parseMessages(raw: unknown): Message[] {
        ...
        if (typeof raw === 'object' && raw !== null) {
            ...
            // ----------------------------------------------------------------------
            // Activity Events (topic: activity / trades)
            // ----------------------------------------------------------------------
            if (obj.topic === 'activity' || obj.type === 'trades' || obj.type === 'orders_matched' || 'market_slug' in obj && 'side' in obj && 'size' in obj && 'asset' in obj) {
                messages.push({
                    topic: 'activity',
